...
bool YoloIInt8Calibrator::getBatch(void *bindings[], const char *names[], int nbBindings)
{
    if(curbatch < batches){
        curbatch ++;
...
std::vector<std::string> YoloIInt8Calibrator::ls(std::string path, long howmany)
{
...
bool YoloTrtNet::CreateNetwork()
{
    trt::trtNetWork::CreateNetwork();
...
bool YoloTrtNet::CreateEngineAndSerialize(long maxWorkspaceSize, BuilderFlag flag, int maxBatch)
{
    trt::trtNetWork::CreateEngineAndSerialize(maxWorkspaceSize, flag, maxBatch);
    for(int i = 0;i < buildinfo.contextNum;i ++){
        std::vector<TensorInfo> tinfo;
        TensorInfo tmp;
        ...
        tmp.bindingIndex = 2;
        tmp.gridSize = 26;
        tmp.hostBuffer = binds[i].hostBufferBinds[2].get();
        tmp.masks = {3,4,5};
        tmp.stride = 16;
        tmp.volume = tmp.gridSize
                * tmp.gridSize
        ...
        tmp.bindingIndex = 3;
        tmp.gridSize = 52;
        tmp.hostBuffer = binds[i].hostBufferBinds[3].get();
        tmp.masks = {0,1,2};
        tmp.stride = 8;
        tmp.volume = tmp.gridSize
                * tmp.gridSize
                * (tmp.numBBoxes * (5 + tmp.numClasses));
        tinfo.push_back(tmp);
        m_OutputTensors.push_back(std::move(tinfo));
    }
}

bool YoloTrtNet::EngineDeserialize()
{
    trt::trtNetWork::EngineDeserialize();
    for(int i = 0;i < buildinfo.contextNum;i ++){
        std::vector<TensorInfo> tinfo;
        TensorInfo tmp;
        tmp.anchors = {10,13,  16,30,  33,23,  30,61,  62,45,  59,119,  116,90,  156,198,  373,326};
...
std::vector<BBoxInfo> YoloTrtNet::nonMaximumSuppression(const float nmsThresh, std::vector<BBoxInfo> binfo)
{
...
std::vector<BBoxInfo> YoloTrtNet::nmsAllClasses(const float nmsThresh, std::vector<BBoxInfo> &binfo, const uint numClasses)
{
...
void YoloTrtNet::convertBBoxImgRes(const float scalingFactor, const float &xOffset, const float &yOffset, BBox &bbox)
{
    // Undo Letterbox
    bbox.x1 -= xOffset;
    bbox.x2 -= xOffset;
...
BBox YoloTrtNet::convertBBoxNetRes(const float &bx, const float &by, const float &bw, const float &bh, const uint &stride, const uint &netW, const uint &netH)
{
    BBox b;
...
void YoloTrtNet::addBBoxProposal(const float bx, const float by, const float bw, const float bh, const uint stride, const float scalingFactor, const float xOffset, const float yOffset, const int maxIndex, const float maxProb, std::vector<BBoxInfo> &binfo)
{
    BBoxInfo bbi;
...
std::vector<BBoxInfo> YoloTrtNet::decodeTensor(const int imageIdx, const int imageH, const int imageW, const TensorInfo &tensor)
{
    float scalingFactor
